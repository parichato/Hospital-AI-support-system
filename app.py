...
with tab4:
    ...
    st.caption("สรุปแนวโน้มผู้บาดเจ็บจากระบบ AI เพื่อใช้วางแผนเชิงกลยุทธ์และพัฒนาคุณภาพบริการ")

    # ======================================================
    # 🗂️ 1. Load Log File (ประวัติการทำนาย)
    # ======================================================
    ...
    # ======================================================
    # 🧑‍⚕️ 4. Gender vs Severity (Heatmap)
    # ======================================================
    if "sex" in df_log.columns and total_cases > 0:
        st.markdown("### 🧠 ความสัมพันธ์ระหว่างเพศและระดับความรุนแรง")
        fig, ax = plt.subplots(figsize=(5, 3))
        pivot = pd.crosstab(df_log["sex"], df_log["predicted_severity"])
        sns.heatmap(pivot, annot=True, fmt="d", cmap="coolwarm", ax=ax)
        ax.set_xlabel("ระดับความรุนแรง")
        ax.set_ylabel("เพศ")
        ax.set_title("เพศ × ระดับความรุนแรง")
        st.pyplot(fig)

    # ======================================================
    # ⏳ 5. Trend over Time (Timeline Chart)
    # ======================================================
    if "timestamp" in df_log.columns and not df_log.empty:
        st.markdown("### 📈 แนวโน้มจำนวนเคสตามเวลา")
        df_log["timestamp"] = pd.to_datetime(df_log["timestamp"], errors="coerce")
        df_log = df_log.dropna(subset=["timestamp"])
        daily = df_log.groupby(df_log["timestamp"].dt.date)["predicted_severity"].value_counts().unstack(fill_value=0)
        st.line_chart(daily)
