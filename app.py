...
# ----------------------------------------------------------
# 🧩 TAB 3 — Apriori (Readable Medical-Business Version)
# ----------------------------------------------------------
with tab3:
    st.subheader("🧩 Risk Association Analysis (Apriori Rules)")
    st.caption("วิเคราะห์ความสัมพันธ์ของพฤติกรรมเสี่ยง เพื่อวางแผนป้องกันและสนับสนุนการตัดสินใจเชิงนโยบาย")
    ...
    if model and submit:
        ...
        if not df_rules.empty:
            # 🔹 แปลงข้อความให้อ่านง่าย
            df_rules["antecedents"] = df_rules["antecedents"].apply(decode_set)
            df_rules["consequents"] = df_rules["consequents"].apply(decode_set)

            # 🔹 แสดง Insight สรุปบนสุด
            top_rule = df_rules.iloc[0]
            st.markdown(
                f"""
                <div style='background-color:#222;border-radius:10px;padding:15px;margin-bottom:10px'>
                💡 <b>Insight:</b> ผู้ที่มี <b style='color:#FFC107'>{top_rule['antecedents']}</b> 
...
# ----------------------------------------------------------
# 📊 TAB 4 — Clinical Summary & Insights Dashboard (Updated)
# ----------------------------------------------------------
with tab4:
    ...
    # ======================================================
    # 🗂️ 1. Load Log File (บันทึกการทำนาย)
    # ======================================================
    log_file = "prediction_log.csv"
    if os.path.exists(log_file):
        df_log = pd.read_csv(log_file)
        st.success("📁 โหลดข้อมูลสรุปจากการทำนายเรียบร้อยแล้ว")
    else:
        st.warning("⚠️ ยังไม่มีข้อมูลบันทึกจากการทำนาย")
        df_log = pd.DataFrame(columns=["timestamp", "age", "sex", "predicted_severity", "prov", "is_night",
                                       "risk1","risk2","risk3","risk4","risk5"])
    ...
    avg_age = df_log["age"].mean() if "age" in df_log and not df_log.empty else 0
    ...
    if total_cases > 0:
        ...
        colors = ['#4CAF50', '#FFC107', '#F44336']  # เขียว เหลือง แดง
        df_log['predicted_severity'].value_counts().plot.pie(
            autopct='%1.1f%%', startangle=90, colors=colors, ax=ax, textprops={'color': 'white'}
        )
        ax.set_ylabel('')
        ax.set_title("ระดับความรุนแรงของผู้บาดเจ็บ", color='white')
        st.pyplot(fig)
    ...
    if all(col in df_log.columns for col in risk_cols):
        ...
        ax.set_title("ปัจจัยเสี่ยงที่พบบ่อยที่สุด", color="white")
    ...
    if "prov" in df_log.columns and not df_log.empty:
        ...
        ax.set_title("Top 5 จังหวัดที่พบเหตุบ่อย", color="white")
        st.pyplot(fig)
    else:
    ...
    # ======================================================
    st.markdown("### ⏱️ ช่วงเวลาที่เกิดเหตุบ่อยที่สุด")
    if "is_night" in df_log.columns:
        ...
        fig, ax = plt.subplots(figsize=(4,4))
        time_counts.plot.pie(autopct='%1.1f%%', colors=["#1976D2","#512DA8"], startangle=90, ax=ax)
        ax.set_ylabel("")
        ax.set_title("สัดส่วนช่วงเวลาที่เกิดเหตุ", color="white")
        st.pyplot(fig)
    ...
    if total_cases > 0:
        ...
        if top_severity == "เสี่ยงมาก":
            msg = "พบแนวโน้มผู้บาดเจ็บรุนแรงสูง ควรจัดสรรทีมฉุกเฉินและทรัพยากรเพิ่มในพื้นที่เสี่ยง"
        elif top_severity == "เสี่ยงปานกลาง":
            msg = "ผู้บาดเจ็บส่วนใหญ่ความรุนแรงปานกลาง เน้นติดตามอาการและประเมินซ้ำ"
        else:
            msg = "กลุ่มความเสี่ยงต่ำเป็นส่วนใหญ่ — มุ่งเน้นมาตรการป้องกันและให้ความรู้ประชาชน"

        st.info(f"📊 สรุปสถานการณ์ปัจจุบัน: {msg}")
        st.caption("💡 ใช้ข้อมูลนี้เพื่อสนับสนุนการวางแผนเชิงกลยุทธ์และพัฒนาระบบบริการสุขภาพ")
